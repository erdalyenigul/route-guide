...
const galleryGroups = computed(() =>
  store
    .stopsForRoute(String(route.params.routeId))
    .map((stop, index) => ({
      id: stop.id,
      index,
      title: stop.title,
...
const selectedGroup = computed(() =>
  galleryGroups.value.find((group) => group.id === selectedStopId.value)
)
const photos = computed(
  () =>
    selectedGroup.value?.photos.map((photo) => ({
      photo,
      stopTitle: selectedGroup.value?.title ?? ''
    })) ?? []
)
    ...
    >
      ...
      >
        <img
          ...
          :alt="t('gallery.photoOf', { stop: t(group.title) })"
          loading="lazy"
        />
        ...
        <span class="group-overlay">
          <span
            ...
            ><small>{{ t(group.region) }}</small></span
          >
          <span class="photo-count"
            ><v-icon icon="mdi-image-multiple-outline" />{{
              t('gallery.photoCount', { count: group.photos.length })
            }}</span
          >
        </span>
      </button>
...
.gallery-group:after {
  ...
  position: absolute;
  inset: 20% 0 0;
  background: linear-gradient(transparent, rgba(4, 5, 8, 0.94));
  pointer-events: none;
}
.gallery-group > img {
  display: block;
...
.group-overlay > span:first-child {
  display: grid;
  gap: 4px;
  min-width: 0;
}
.group-overlay strong {
  overflow: hidden;
  font-size: 1.22rem;
...
.photo-grid button:after {
  ...
  position: absolute;
  inset: 35% 0 0;
  background: linear-gradient(transparent, rgba(2, 8, 5, 0.74));
  pointer-events: none;
}
.photo-grid img {
  width: 100%;
