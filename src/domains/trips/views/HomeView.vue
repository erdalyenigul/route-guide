  ...
  <main class="page-shell trip-page">
    ...
    <template v-else>
      <section class="trip-summary">
        <header>
          <div><p>{{ t('home.title') }}</p><h1>{{ text(trip.title) }}</h1></div>
          <v-btn class="open-map-action" color="primary" rounded="pill" :to="`/trips/${routeId}/map`">
        ...
        <div class="summary-grid">
          ...
          <div><span>{{ t('home.remainingDistance') }}</span><strong>{{ store.remainingDistance }} <small>{{ t('common.km') }}</small></strong></div>
            ...
            >
              ...
              >
                ...
                <span class="chapter-title">
                  ...
                  <span v-else-if="stop.status === 'skipped'" class="stage-complete-label skipped"><v-icon icon="mdi-skip-next-circle" />{{ t('stop.routeSkipped') }}</span>
                </span>
                <v-icon
                  v-if="stop.favorite"
                ...
                <div v-show="openChapters.includes(stop.id)" :id="`chapter-${stop.id}`" class="drawer-content">
                  <div class="drawer-status">
                    <v-chip size="small" :color="chapterStatus(stop,index)==='current'?'secondary':chapterStatus(stop,index)==='completed'?'success':undefined">
                      {{ chapterStatus(stop,index)==='completed' ? t('stop.stageComplete') : chapterStatus(stop,index)==='skipped' ? t('stop.routeSkipped') : t(`common.${chapterStatus(stop,index)}`) }}
                    </v-chip>
                  </div>
