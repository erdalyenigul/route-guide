    ...
    <template v-else>
      ...
      <section class="chapters">
        ...
        <div class="chapter-grid">
          <article
            v-for="(stop,index) in store.activeStops"
            :key="stop.id"
            class="chapter"
          ...
          >
            ...
            >
              ...
              <span class="chapter-title"><strong>{{ t(stop.title) }}</strong><small>{{ t(stop.region) }}</small></span>
            ...
            <v-expand-transition>
              <div v-show="openChapters.includes(stop.id)" :id="`chapter-${stop.id}`" class="drawer-content">
                ...
                <v-btn block variant="tonal" color="primary" append-icon="mdi-arrow-right" :to="`/trips/${routeId}/stops/${stop.id}`">
                  {{ t('common.details') }}
                </v-btn>
              </div>
            </v-expand-transition>
...
.chapters{margin-top:42px}.section-heading{display:flex;align-items:end;justify-content:space-between;margin-bottom:18px}.section-heading h2{font-size:1.75rem;letter-spacing:-.04em}.section-heading>span{display:grid;min-width:36px;height:36px;place-items:center;border:1px solid rgba(var(--v-theme-primary),.2);border-radius:50%;color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.1);font-size:.9rem;font-weight:720}.chapter-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));align-items:start;gap:14px}.chapter{position:relative;min-width:0;overflow:hidden;border:1px solid rgba(var(--chapter-accent),.23);border-radius:var(--app-radius-md);background:radial-gradient(circle at 0 0,rgba(var(--chapter-accent),.14),transparent 52%),linear-gradient(145deg,rgba(var(--chapter-accent),.045),rgb(var(--v-theme-surface)) 65%);box-shadow:0 9px 30px rgba(0,0,0,.07);transition:border-color .2s,box-shadow .2s,transform .2s}.chapter:before{content:'';position:absolute;z-index:1;left:0;top:0;width:100%;height:4px;background:linear-gradient(90deg,rgb(var(--chapter-accent)),rgba(var(--chapter-accent),.2))}.chapter:hover,.chapter.open{border-color:rgba(var(--chapter-accent),.62);box-shadow:0 20px 52px rgba(var(--chapter-accent),.14);transform:translateY(-2px)}.chapter.current{border-color:rgba(var(--v-theme-secondary),.58)}.chapter.completed{filter:saturate(.8)}.drawer-summary{display:grid;width:100%;grid-template-columns:52px minmax(0,1fr) 28px;align-items:center;gap:14px;padding:22px 20px 20px;border:0;color:inherit;text-align:left;background:transparent;cursor:pointer}.chapter-id{display:grid;gap:4px;color:rgb(var(--chapter-accent));font-size:1.08rem;font-weight:860;letter-spacing:.04em;text-shadow:0 0 24px rgba(var(--chapter-accent),.22)}.chapter-id small{color:rgba(var(--chapter-accent),.78);font-size:.62rem;font-weight:780;letter-spacing:.1em;text-transform:uppercase}.chapter-title{display:grid;min-width:0;gap:5px}.chapter-title strong{overflow:hidden;font-size:1.14rem;letter-spacing:-.025em;text-overflow:ellipsis;white-space:nowrap}.chapter-title small{overflow:hidden;color:rgba(var(--v-theme-on-surface),.62);font-size:.82rem;text-overflow:ellipsis;white-space:nowrap}.drawer-chevron{justify-self:end;color:rgb(var(--chapter-accent));transition:transform .22s}.chapter.open .drawer-chevron{transform:rotate(180deg)}.drawer-content{padding:0 20px 20px;border-top:1px solid rgba(var(--chapter-accent),.16)}.drawer-status{display:flex;justify-content:flex-end;margin-top:16px}.overview{margin-top:14px;color:rgba(var(--v-theme-on-surface),.74);font-size:.94rem;line-height:1.6}.chapter-data{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:18px 0;padding:17px 0;border-top:1px solid rgba(var(--chapter-accent),.14);border-bottom:1px solid rgba(var(--chapter-accent),.14)}.chapter-data span,.chapter-data strong{display:block}.chapter-data span{color:rgba(var(--chapter-accent),.84);font-size:.75rem}.chapter-data strong{margin-top:4px;font-size:.9rem}.drawer-content .v-btn{background:rgba(var(--chapter-accent),.12)!important;color:rgb(var(--chapter-accent))!important}
