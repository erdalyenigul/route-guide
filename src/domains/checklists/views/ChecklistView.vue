  ...
  <div class="page-shell standard-page narrow-page">
    ...
    <v-card class="progress-card pa-6">
      <div class="d-flex justify-space-between mb-3">
        <strong>{{ t('checklist.progress', { done, total: store.checklist.length }) }}</strong
        ><span>{{ store.checklistProgress }}%</span>
      </div>
      <v-progress-linear
        :model-value="store.checklistProgress"
        color="secondary"
        height="8"
        rounded
      />
    ...
    <div class="checklist-list">
      <button
        ...
        @click="store.toggleChecklistItem(item.id)"
      >
...
.check-item.completed > span:nth-child(2) {
