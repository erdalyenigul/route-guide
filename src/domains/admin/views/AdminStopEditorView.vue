...
const body = computed({
  ...
  set: (value: string) => {
    experienceDraft.value.body = value
  }
})
const isPublished = computed({
  get: () => experienceDraft.value.isPublished,
  set: (value: boolean) => {
    experienceDraft.value.isPublished = value
  }
})
const caption = ref('')
...
const tripStopIndex = computed(() =>
  tripStops.value.findIndex((stop) => stop.id === tripStop.value?.id)
)
...
const uploadedPhotoCount = computed(
  () => editor.value?.photos.filter((photo) => photo.sourceType === 'trip').length ?? 0
)
const remainingPhotos = computed(() => MAX_PHOTOS_PER_STOP - uploadedPhotoCount.value)
const viewerPhotos = computed(
  () =>
    editor.value?.photos.map((photo) => ({
      id: photo.id,
      url: photo.url,
...
async function saveProgress(): Promise<void> {
  ...
  const resolvedNights = isAccommodationStop.value
    ? Math.min(Math.max(Math.trunc(Number(nightsStayed.value) || 0), 0), 365)
    : null
  const resolvedDistance = isRouteOrigin.value
    ? null
    : Math.min(Math.max(Math.trunc(Number(actualDistanceKm.value) || 0), 0), 5000)
  try {
...
async function upload(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value =
      error instanceof Error && error.message === 'PHOTO_LIMIT_REACHED'
...
<template>
  <main class="page-shell standard-page editor-page">
    ...
    <template v-else-if="editor">
      ...
      >
        <v-btn
          :to="`/trips/${trip.activeTrip?.id ?? 'active'}/stops/${stopSlug}`"
          variant="tonal"
          prepend-icon="mdi-open-in-new"
          >{{ t('admin.viewStop') }}</v-btn
        >
      </PageHeader>
      ...
      <div class="editor-sections">
        <section class="editor-section">
          ...
          <div class="section-content">
            <div
              v-if="editor.photos.length"
              class="photo-admin-grid"
            >
              <v-card
              ...
              >
                ...
                >
                  <img
                    :src="photo.url"
                    :alt="photo.caption || t('gallery.photoOf', { stop: t(editor.titleKey) })"
                  />
                </button>
            ...
            <v-empty-state
              ...
              :title="t('admin.noPhotos')"
              :text="t('admin.noPhotosHint')"
            />
          </div>
        </section>
          ...
          <div class="section-content">
            <div class="drawer-content">
              ...
              <v-btn
                color="primary"
                size="large"
                :disabled="!files.length || remainingPhotos === 0"
                :loading="isUploading"
                @click="upload"
                >{{ t('admin.uploadPhotos') }}</v-btn
              >
            </div>
          </div>
        </section>
              ...
              <div class="planned-values">
                ...
                <div v-if="!isRouteOrigin">
                  <v-icon icon="mdi-map-marker-distance" /><span>{{
                    t('admin.plannedDistance')
                  }}</span
                  ><strong
                    >{{ tripStop.drivingDistanceFromPreviousKm ?? '—' }}
                    {{ t('common.km') }}</strong
                  >
                </div>
              </div>
        ...
        <section class="editor-section">
          ...
          <div class="section-content">
            <div class="drawer-content">
              ...
              />
              <p
                v-if="experienceDraft.authorName && experienceDraft.updatedAt"
                class="author-meta"
              >
                <v-icon icon="mdi-account-edit-outline" />
                {{
                  t('admin.lastEditedBy', {
                    name: experienceDraft.authorName,
                    date: formatDateTime(experienceDraft.updatedAt)
                  })
                }}
              </p>
              <v-btn
                class="experience-save"
                color="primary"
                size="large"
                :loading="isSaving"
    ...
    <v-dialog
      ...
      @update:model-value="
        (value) => {
          if (!value) pendingDelete = undefined
        }
      "
    >
