...
async function adminUser(user: User): Promise<AdminUser> {
  const { data, error } = await client().from('profiles').select('username,display_name,is_editor').eq('id', user.id).maybeSingle()
  ...
  const displayName = data?.display_name
    ?? String(user.user_metadata.display_name ?? username)
...
async function stopIdForSlug(stopSlug: string): Promise<{ id: string; titleKey: string; contentKey: string }> {
  const { data, error } = await client().from('stops').select('id,title_key,content_key').eq('slug', stopSlug).single()
...
export const supabaseAdminContentRepository: AdminContentRepository = {
  ...
  async uploadPhoto(stopSlug, image, caption) {
    ...
    const insertResult = await client().from('galleries').insert({
      route_id: null,
      stop_id: stop.id,
  ...
  async deletePhoto(photoId) {
    ...
    if (data.is_cover && data.stop_id) {
      ...
      if (replacement) {
        const promoteResult = await client().from('galleries').update({ is_cover: true }).eq('id', replacement.id)
  ...
  async setCover(stopSlug, photoId) {
    await requireUser()
    const stop = await stopIdForSlug(stopSlug)
    const clearResult = await client().from('galleries').update({ is_cover: false }).eq('stop_id', stop.id)
    if (clearResult.error) throw clearResult.error
    const coverResult = await client().from('galleries').update({ is_cover: true }).eq('id', photoId).eq('stop_id', stop.id)
