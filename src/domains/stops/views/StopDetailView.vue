...
const experienceLocale = ref<'en' | 'tr'>(locale.value === 'tr' ? 'tr' : 'en')
const publishedExperienceLocales = computed(() => (['tr', 'en'] as const).filter(language => {
  const experience = stop.value?.experiences?.[language]
  return experience?.isPublished && experience.body.trim()
}))
const localizedExperience = computed(() => {
  if (!stop.value) return undefined
  return stop.value.experiences?.[experienceLocale.value]
})
...
watch([locale, () => stop.value?.id, publishedExperienceLocales], () => {
  const preferredLocale = locale.value === 'tr' ? 'tr' : 'en'
  experienceLocale.value = publishedExperienceLocales.value.includes(preferredLocale)
    ? preferredLocale
    : (publishedExperienceLocales.value[0] ?? preferredLocale)
}, { immediate: true })
...
const detailSections = computed<DetailSection[]>(() => {
  ...
  if (hasPublishedExperience.value) {
    sections.push({ key: 'experience', icon: 'mdi-book-open-page-variant-outline', title: 'stop.ourExperience', badge: experienceLocale.value.toUpperCase() })
  }
          ...
          <v-expansion-panel v-for="section in column" :key="section.key" :value="section.key" :class="{ 'experience-panel': section.key === 'experience' }">
            ...
            <v-expansion-panel-text>
              ...
              <template v-else-if="section.key === 'experience'">
                <v-btn-toggle
                  v-if="publishedExperienceLocales.length > 1"
                  v-model="experienceLocale"
      ...
      <section class="completion-section">
        ...
        >
          <span class="stage-action-icon">
            <v-icon :icon="stop.status === 'visited' ? 'mdi-check-circle' : 'mdi-flag-checkered'" />
          </span>
