...
function level(value: string | null): string { return value === null ? t('common.unknown') : t(`common.${value}`) }
...
<template>
  <div class="conditions-grid">
    <div v-for="metric in metrics" :key="metric.key" class="condition-item">
      <v-icon :icon="metric.icon" size="21" />
      <div><span>{{ t(`stop.${metric.key}`) }}</span><strong>{{ metric.value }}</strong></div>
    </div>
  </div>
</template>

<style scoped>
.conditions-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.condition-item { display: flex; align-items: center; gap: 12px; padding: 15px; border-radius: 16px; background: rgba(var(--v-theme-on-surface), .04); }
.condition-item .v-icon { color: rgb(var(--v-theme-primary)); }.condition-item span, .condition-item strong { display: block; }.condition-item span { font-size: .72rem; color: rgba(var(--v-theme-on-surface), .58); }.condition-item strong { margin-top: 2px; font-size: .9rem; }
@media (min-width: 700px) { .conditions-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
</style>
