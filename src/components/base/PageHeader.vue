...
<template>
  <header
    class="page-header"
    :class="{ 'has-back': back, 'stack-actions': stackActions }"
  >
    <v-btn
      v-if="back"
      class="back-btn"
...
.back-btn {
  ...
  background: rgba(var(--v-theme-on-background), 0.065);
...
.has-back > div:not(.page-actions) {
  min-width: 0;
  grid-column: 1 / -1;
  grid-row: 2;
}
.has-back .page-actions {
  grid-column: 1 / -1;
  grid-row: 3;
  justify-self: start;
  margin: 0;
...
.stack-actions .page-actions :deep(.v-btn) {
  white-space: nowrap;
}
