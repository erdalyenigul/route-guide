...
<style scoped>
.photo-viewer{position:relative;display:grid;place-items:center;background:#07080a!important;touch-action:pan-y}.viewer-window{width:100%;height:100%;background:transparent!important}.viewer-window :deep(.v-window__container),.viewer-window :deep(.v-window-item){height:100%}.viewer-slide{display:grid;width:100%;height:100%;place-items:center}.viewer-slide img{width:100%;height:100%;max-height:100dvh;object-fit:contain;user-select:none;-webkit-user-drag:none}.photo-viewer>.v-btn{position:fixed!important;z-index:20;top:auto;right:auto;min-width:52px;min-height:52px;color:#fff!important;background:#090a0d!important;border:1px solid rgba(255,255,255,.18);box-shadow:0 10px 28px rgba(0,0,0,.42)!important;backdrop-filter:blur(16px)}.photo-viewer>.v-btn:disabled{opacity:.3}.viewer-close{top:max(18px,env(safe-area-inset-top));right:18px}.viewer-previous,.viewer-next{top:50%;transform:translateY(-50%)}.viewer-previous{left:max(14px,env(safe-area-inset-left))}.viewer-next{right:max(14px,env(safe-area-inset-right))}.photo-viewer>p{position:absolute;z-index:2;right:20px;bottom:max(24px,env(safe-area-inset-bottom));left:20px;padding:10px 14px;border-radius:12px;color:#fff;text-align:center;background:rgba(0,0,0,.58);backdrop-filter:blur(14px)}
@media(max-width:600px){.photo-viewer>.v-btn{min-width:48px;min-height:48px}.viewer-close{top:max(14px,env(safe-area-inset-top));right:14px}.viewer-previous{left:8px}.viewer-next{right:8px}}
</style>
