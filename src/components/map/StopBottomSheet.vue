...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <p v-if="preferredCamp" class="navigation-target">{{ t('map.navigationTarget') }}: <strong>{{ t(preferredCamp.title) }}</strong></p>
...
.navigation-target{margin-top:12px;padding:9px 12px;border-radius:12px;color:rgba(var(--v-theme-on-surface),.68);background:rgba(var(--v-theme-primary),.09);font-size:.78rem}.navigation-target strong{color:rgb(var(--v-theme-on-surface))}
</style>
