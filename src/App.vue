...
.mobile-nav{position:fixed;z-index:1005;right:0;bottom:0;left:0;display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr));width:100%;padding:6px 8px calc(6px + env(safe-area-inset-bottom));border-top:1px solid rgba(var(--v-border-color),.1);background:rgba(var(--v-theme-surface),.9)!important;box-shadow:0 -14px 42px rgba(0,0,0,.14)!important;backdrop-filter:blur(24px) saturate(140%)}.mobile-nav .v-btn{width:100%;min-width:0!important;height:66px!important;border-radius:18px!important;color:rgba(var(--v-theme-on-surface),.62);transition:color .18s ease,background .18s ease,transform .14s ease!important}.mobile-nav .v-btn :deep(.v-btn__content){flex-direction:column;gap:4px}.mobile-nav .v-btn .v-icon{font-size:1.45rem;transition:transform .18s ease}.mobile-nav .v-btn span{font-size:.77rem;font-weight:700}.mobile-nav .v-btn.v-btn--active{color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.11)}.mobile-nav .v-btn.v-btn--active .v-icon{transform:translateY(-1px) scale(1.06)}
.page-enter-active,.page-leave-active{transition:opacity .2s ease,transform .2s cubic-bezier(.22,.8,.28,1)}.page-enter-from{opacity:0;transform:translateY(7px) scale(.995)}.page-leave-to{opacity:0;transform:translateY(-2px)}
