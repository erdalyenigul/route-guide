...
export const useTripStore = defineStore('trip', () => {
  ...
  const activeTrip = computed(
    () => dataset.value.routes.find((route) => route.status === 'active') ?? dataset.value.routes[0]
  )
  ...
  async function initialize(): Promise<void> {
    ...
    try {
      ...
      const route =
        dataset.value.routes.find((item) => item.status === 'active') ?? dataset.value.routes[0]
      if (route) {
  ...
  const lastProgressedStopIndex = computed(() =>
    activeStops.value.reduce(
      (latest, stop, index) =>
        stop.status === 'visited' || stop.status === 'skipped' ? index : latest,
      -1
    )
  )
  ...
  const currentStopIndex = computed(() =>
    currentStop.value
      ? activeStops.value.findIndex((stop) => stop.id === currentStop.value?.id)
      : -1
  )
  ...
  const remainingNights = computed(() =>
    accommodationStops.value
      ...
      .reduce((total, stop) => total + stop.recommendedNights, 0)
  )
  const totalDistance = computed(
    () =>
      activeTrip.value?.totalDistanceKm ||
      activeStops.value.reduce(
        (total, stop) => total + (stop.drivingDistanceFromPreviousKm ?? 0),
        0
      )
  )
  const completedDistance = computed(() =>
    activeStops.value
      .filter((stop) => stop.status === 'visited')
      .reduce(
  ...
  const nightsStayed = computed(() =>
    accommodationStops.value
      .filter((stop) => stop.status === 'visited')
      .reduce((total, stop) => total + (stop.nightsStayed ?? 0), 0)
  )
  const routeProgress = computed(() =>
    activeStops.value.length
      ? Math.round(
          (activeStops.value.filter(
            (stop) => stop.status === 'visited' || stop.status === 'skipped'
          ).length /
            activeStops.value.length) *
            100
        )
  ...
  const favoriteStops = computed(() =>
    dataset.value.stops.map(toStopViewModel).filter((stop) => stop.favorite)
  )
  const checklist = computed<ChecklistItemViewModel[]>(() =>
    dataset.value.checklist.map((item) => ({
      ...item,
  ...
  const checklistProgress = computed(() =>
    checklist.value.length
      ? Math.round(
          (checklist.value.filter((item) => item.completed).length / checklist.value.length) * 100
        )
      : 0
  )
  ...
  function syncFailure(error: unknown): void {
    stateSyncError.value =
      error instanceof Error && error.message === 'AUTH_REQUIRED' ? 'auth' : 'save'
  }
  ...
  function campingSpotsForStop(stopId: string) {
    ...
    return ids
      .map((id) => dataset.value.spots.find((spot) => spot.id === id))
  ...
  async function toggleFavorite(stopId: string): Promise<void> {
    ...
    userState.value.favoriteStopIds = favorite
      ? [...favorites, stopId]
  ...
  async function setStopStatus(
    ...
    const staleCurrentStops = advancesRoute
      ? routeStops.filter(
          (item) => item.id !== stopId && item.id !== followingStop?.id && item.status === 'current'
        )
      : []
    const nextNightsStayed =
      isCompleted && nightsStayed !== null ? Math.max(0, Math.trunc(nightsStayed)) : null
    const nextActualDistance =
      isCompleted && actualDistanceKm !== null ? Math.max(0, Math.trunc(actualDistanceKm)) : null
    const shouldPromoteFollowingStop = Boolean(
      advancesRoute &&
      followingStop &&
      followingStatus !== 'visited' &&
      followingStatus !== 'skipped'
    )
    const shouldRestoreFollowingStop = Boolean(
      !advancesRoute && followingStop && followingStatus === 'current'
    )
    userState.value.stopStatuses[stopId] = nextStatus
    if (followingStop && shouldPromoteFollowingStop)
      userState.value.stopStatuses[followingStop.id] = 'current'
    if (followingStop && shouldRestoreFollowingStop)
      userState.value.stopStatuses[followingStop.id] = followingStop.initialStatus
    for (const staleStop of staleCurrentStops)
      userState.value.stopStatuses[staleStop.id] = 'planned'
    ...
    try {
      const writes = [
        tripStateService.setStopProgress(
          routeId,
          stopId,
          nextStatus,
          nextNightsStayed,
      ...
      if (followingStop && followingStatus)
        userState.value.stopStatuses[followingStop.id] = followingStatus
      for (const staleStop of staleCurrentStops)
        userState.value.stopStatuses[staleStop.id] = 'current'
  ...
  function setStopCompletion(
    ...
    return setStopStatus(
      stopId,
      completed ? 'visited' : (stop?.initialStatus ?? 'planned'),
      nightsStayed,
      actualDistanceKm
    )
  }
